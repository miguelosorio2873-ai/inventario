...
from utils.icon_util import icon_users, icon_plus, icon_edit, icon_trash, icon_search, icon_clear, icon_save
...
class PanelClientes(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        ctk.CTkLabel(self, text="Clientes", font=("Segoe UI", 24, "bold"), text_color=COLOR_TEXT, image=icon_users(24, COLOR_TEXT), compound="left").pack(anchor="w", padx=25, pady=(25, 15))
        ...
        if sesion.tiene_permiso_accion("Clientes", "CREAR"):
    ...
    def _crear_tabla(self):
        ...
        for col in ["ID", "Cédula", "Nombre", "Correo", "Teléfono"]:
    ...
    def cargar_datos(self):
        for w in self.scroll.winfo_children():
            w.destroy()
        sesion = SesionUsuario()
        puede_editar = sesion.tiene_permiso_accion("Clientes", "EDITAR")
        puede_eliminar = sesion.tiene_permiso_accion("Clientes", "ELIMINAR")
        for c in self.dao.listar_todos():
            row = ctk.CTkFrame(self.scroll, fg_color="#2d2d2d", corner_radius=4)
            row.pack(fill="x", pady=1)
            for v in [c.id, c.cedula, c.nombre, c.correo, c.telefono]:
    ...
    def buscar(self):
        ...
        puede_editar = sesion.tiene_permiso_accion("Clientes", "EDITAR")
        puede_eliminar = sesion.tiene_permiso_accion("Clientes", "ELIMINAR")
        for c in self.dao.buscar(texto):
            row = ctk.CTkFrame(self.scroll, fg_color="#2d2d2d", corner_radius=4)
            row.pack(fill="x", pady=1)
            for v in [c.id, c.cedula, c.nombre, c.correo, c.telefono]:
                ctk.CTkLabel(row, text=str(v), font=("Segoe UI", 12), text_color=COLOR_TEXT).pack(side="left", padx=10, pady=6)
    ...
    def editar(self, id):
        ...
        c = next((x for x in clientes if x.id == id), None)
        if not c:
            return
        self.editando = c
        self._mostrar_formulario()
    ...
    def _mostrar_formulario(self):
        ...
        ctk.CTkLabel(win, text="Formulario de Cliente", font=("Segoe UI", 18, "bold"), text_color=COLOR_TEXT).pack(pady=15)

        campos = {}
        for label in ["Cédula", "Nombre", "Correo", "Teléfono"]:
        ...
        if self.editando:
            campos["Cédula"].insert(0, self.editando.cedula or "")
            campos["Nombre"].insert(0, self.editando.nombre or "")
            campos["Correo"].insert(0, self.editando.correo or "")
            campos["Teléfono"].insert(0, self.editando.telefono or "")

        def guardar():
            try:
                c = self.editando or Cliente()
                c.cedula = campos["Cédula"].get().strip()
                c.nombre = campos["Nombre"].get().strip()
                c.correo = campos["Correo"].get().strip()
                c.telefono = campos["Teléfono"].get().strip()
                if self.editando:
